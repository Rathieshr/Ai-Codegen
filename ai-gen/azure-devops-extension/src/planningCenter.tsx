...
export function PlanningCenter({
  ...
  if (!selected || !workspaceActions) return <section className="hei-planning-center">{proposalContinuation}<div className="hei-planning-empty"><strong>No implementation plan is available.</strong><span>{currentProposal ? 'Continue the current plan to review its work items before creation.' : 'Start with a Requirement. HEI will prepare an implementation plan for your review.'}</span>{currentProposal ? <button className="planner-button primary" type="button" onClick={continueRequirementPlanning}>Continue Implementation Plan</button> : null}</div></section>;
  ...
  const hierarchy = hierarchyLoading && !hierarchyData
    ? <div className="hei-planning-overview-state"><strong>Loading plan...</strong><span>Preparing the selected work-item hierarchy.</span></div>
    : hierarchyError
...
function PlanningProposalContinuation({ value, onContinue }: { value: PlanningProposalSummary; onContinue: () => void }) {
  ...
  return <section className="hei-current-proposal" aria-label="Current Implementation Plan">
    <div><span>Current Implementation Plan</span><h3>{value.title}</h3><p>{included} work item{included === 1 ? '' : 's'} · v{value.version} · {next}</p></div>
    <div><span className={`hei-planning-status status-${value.status.toLowerCase()}`}>{value.status}</span><button className="planner-button primary" type="button" onClick={onContinue}>Continue Plan</button></div>
  </section>;
}
