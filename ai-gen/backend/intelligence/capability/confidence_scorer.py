...
def score_candidates(candidates: list[dict], threshold: int = DEFAULT_DISCOVERY_THRESHOLD) -> list[dict]:
    ...
    for candidate in candidates:
        overall = (
            candidate.get("intent_score", 0) * 0.28
            + candidate.get("keyword_score", 0) * 0.12
            + candidate.get("business_goal_score", 0) * 0.20
            + candidate.get("knowledge_score", 0) * 0.18
            + candidate.get("repository_score", 0) * 0.16
            + candidate.get("memory_score", 0) * 0.06
        )
        synergy = 0
    ...
    return sorted(scored, key=lambda item: (item.get("overall_score", 0), item.get("intent_score", 0), item.get("knowledge_score", 0)), reverse=True)
