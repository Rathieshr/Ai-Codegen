...
export function HEIApplication({ hostAdapter = adapter }: { hostAdapter?: HEIHostAdapter }) {
  ...
  useEffect(() => {
    ...
    hostAdapter.initialize().then(async (hostContext) => {
      ...
      const loaded = await requestWorkspace(hostContext).catch(() => fallbackWorkspace(hostContext));
      ...
      await recordDiagnostic(hostContext, 'HubLoaded', { startupMs: Math.round(performance.now() - started), theme: hostContext.theme, hostType: hostContext.hostType });
      await recordDiagnostic(hostContext, 'WorkspaceLoaded', { workspaceId: loaded.workspaceId, projectId: hostContext.project.id, userId: hostContext.user.id });
      setBusy(false);
    }).catch((reason) => {
  ...
  async function updatePreferences(changes: Partial<WorkspacePreferences>) {
    ...
    try {
      const response = await fetch(`${BASE_URL}/workspace/preferences`, {
  ...
  async function loadOverview(hostContext: HEIHostContext) {
    try {
      ...
      const response = await fetch(`${BASE_URL}/dashboard/overview${query}`);
      ...
      setOverview(value);
      setWorkspace((current) => current ? { ...current, notifications: dashboardNotifications(value) } : current);
    } catch (reason) { setError(message(reason)); }
  }

  if (busy || !context) return <div className="hei-hub-loading" role="status"><strong>Starting HEI</strong><span>Resolving Azure DevOps workspace context...</span></div>;
  ...
  return (
    ...
    >
      ...
      <Suspense fallback={<div className="hei-hub-route-loading" role="status">Loading {routeLabel(route)}...</div>}>
        {route === 'overview' ? <OverviewDashboard overview={overview} loading={busy} onRefresh={() => void loadOverview(context)} /> : null}
        ...
        {route === 'repository' ? <RepositoryCenter baseUrl={BASE_URL} preferredRepositoryId={context.repository.id} actor={actor} canManage={canAdmin} onError={reportError} /> : null}
        ...
        {route === 'settings' ? <SettingsWorkspace context={context} /> : null}
      </Suspense>
    </EngineeringCommandCenterShell>
  );
}
...
async function requestWorkspace(context: HEIHostContext): Promise<EngineeringWorkspace> {
  ...
  const response = await fetch(`${BASE_URL}/workspace?${query.toString()}`, { headers: { 'X-HEI-User': context.user.id } });
...
async function recordDiagnostic(context: HEIHostContext, eventType: string, metadata: Record<string, unknown>) {
  try {
    await fetch(`${BASE_URL}/workspace/diagnostics`, {
...
function navigationFor(role: string): WorkspaceNavigationItem[] {
  ...
  return [
    ...
    ...(canAdmin ? [item('settings', 'Settings', 'settings')] : []),
  ];
}
