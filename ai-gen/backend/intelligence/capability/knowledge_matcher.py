...
from .capability_rules import (
    ...
    capability_business_priority,
    capability_classification,
)

GENERIC_BUSINESS_TOKENS = {"status", "visibility", "review", "manage", "monitoring", "operations", "center", "workspace"}
...
def discover_knowledge_candidates(intent_summary: dict[str, Any], knowledge_registry: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    expanded = [item.lower() for item in _list(intent_summary.get("expandedConcepts"))]
    ...
    for capability in CORE_CAPABILITIES:
        ...
        expansion_hits = [concept for concept in expanded if any(concept in keyword.lower() or keyword.lower() in concept for keyword in keywords)]
        ...
        business_hits = [
            token
            for token in intent_summary.get("domainNouns", [])
            if token.lower() not in GENERIC_BUSINESS_TOKENS
            and any(token.lower() in keyword.lower() or keyword.lower() in token.lower() for keyword in keywords)
        ]
        if not any([keyword_hits, expansion_hits, business_hits, capability == primary, capability in secondary]):
            continue
        intent_score = min(100, 40 + len(keyword_hits) * 12 + (28 if capability == primary else 0) + (16 if capability in secondary else 0))
        keyword_score = min(100, 35 + len(keyword_hits) * 15 + len(business_hits) * 8 + len(expansion_hits) * 6)
        knowledge_score = min(100, 35 + len(module_hits) * 18 + len(flow_hits) * 18)
        business_goal_score = min(100, 30 + len(business_hits) * 18 + len(keyword_hits) * 10 + len(expansion_hits) * 7)
        domain_expansion_score = min(100, 25 + len(expansion_hits) * 16 + len(keyword_hits) * 6) if expansion_hits else 0
        evidence = _unique([*keyword_hits, *expansion_hits, *module_hits, *flow_hits, *business_hits])[:8]
        candidates.append(
            {
                "name": capability,
                "classification": capability_classification(capability),
                "business_priority": capability_business_priority(capability),
                "intent_score": intent_score,
                "keyword_score": keyword_score,
                "knowledge_score": knowledge_score,
                "business_goal_score": business_goal_score,
                "domain_expansion_score": domain_expansion_score,
                "repository_score": 0,
                "memory_score": 0,
                "module_hits": module_hits[:4],
                "flow_hits": flow_hits[:4],
                "evidence": evidence,
                "knowledge_evidence": _unique([*module_hits, *flow_hits, *expansion_hits])[:6],
